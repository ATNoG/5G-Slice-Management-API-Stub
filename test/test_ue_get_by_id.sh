...
# @Author: Test Suite
# @Date:   2025-11-05
# Test script for GET /UE/{id}/get endpoint
...
echo "=================================================="
echo "Testing GET /UE/{id}/get - Get UE by ID"
echo "=================================================="
echo ""
...
echo -e "${GREEN}Test 1: HTTP 200 - Successful retrieval of UE by ID${NC}"
echo "-------------------------------------------"
curl -s --request GET "http://${BASE_URL}/UE/1/get" \
...
# Test 3: HTTP 404 - ID not found
echo -e "${YELLOW}Test 3: HTTP 404 - UE ID does not exist${NC}"
echo "-------------------------------------------"
curl -s --request GET "http://${BASE_URL}/UE/999/get" \
--header "${AUTH_HEADER}" | jq .
echo ""
echo ""

# Test 4: HTTP 400 - Bad request
echo -e "${RED}Test 4: HTTP 400 - Bad request (invalid ID)${NC}"
echo "-------------------------------------------"
curl -s --request GET "http://${BASE_URL}/UE/-1/get" \
--header "${AUTH_HEADER}" | jq .
echo ""
